  ...
  return (
    <div className="md:row-span-1 xl:row-span-2 bg-white dark:bg-gray-800 col-span-1 shadow-md rounded-2xl flex flex-col justify-between">
      ...
      <div className="flex mb-6 items-center justify-around gap-4 px-5">
        <div className="rounded-full p-5 bg-blue-50 dark:bg-blue-900/30 border-sky-300 dark:border-blue-700 border-[1px]">
          {primaryIcon}
        </div>
        <div className="flex-1">
          {details.map((detail, index) => (
            <React.Fragment key={index}>
              ...
              {index < details.length - 1 && <hr className="dark:border-gray-700" />}
            </React.Fragment>
          ))}
        </div>
      </div>
