...
(async () => {
  ...
  try {
    ...
    page.on('console', msg => { try { logs.push({ type: 'console', text: msg.text(), args: msg.args().map(a=>String(a)) }); } catch(e) { logs.push({type:'console', text: String(msg) }) } });
    ...
    try { await page.goto('http://localhost:3000/users', { waitUntil: 'networkidle2', timeout: 30000 }); } catch (e) { logs.push({ type: 'gotoError', text: String(e) }); }
    try { await page.evaluate(() => new Promise(r => setTimeout(r, 2000))); } catch(e) { }
    ...
    const snapshot = await page.evaluate(() => {
      ...
      const navBg = nav ? window.getComputedStyle(nav).backgroundColor : null;
      const bodyBg = window.getComputedStyle(document.body).backgroundColor;
      return { usersText, bodyText, hasDark, navBg, bodyBg };
    });
